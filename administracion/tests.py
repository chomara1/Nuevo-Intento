...
2) AdminRequiredDecoratorTests   -> el decorador admin_required, probado
                                     directamente sobre una vista de ejemplo
                                     (sin necesidad de recorrer toda la app).
3) DashboardVistaTests           -> vista dashboard.
4) AprobarRechazarProveedorTests -> vistas aprobar_proveedor / rechazar_proveedor.
5) ListaYProductosProveedorTests -> lista_proveedores / productos_proveedor.
6) EliminarProveedorYProductoTests -> eliminar_proveedor / eliminar_producto_admin.
7) GestionDisenoVistaTests       -> vista gestion_diseno (formulario del carrusel).
8) PagosYPedidosVistaTests       -> pagos / pedidos / actualizar_estado_pedido.
...
class AdminRequiredDecoratorTests(TestCase):
    """
    En vez de probar el decorador a través de una vista real (lo que ya
    hacemos indirectamente en los tests de vistas de abajo), aquí lo
    probamos de forma aislada envolviendo una vista mínima de prueba con
    RequestFactory. Esto nos permite verificar el decorador en sí mismo,
    sin depender de ninguna plantilla HTML.
    """
    ...
    def setUp(self):
        ...
        self.vista_de_prueba = vista_de_prueba

    def _preparar_request(self, request, user):
        # admin_required usa messages y (indirectamente) la sesión, así que
        # las middlewares de sesión/mensajes deben inicializarse a mano
        # cuando se construye el request manualmente con RequestFactory.
    ...
    def test_usuario_anonimo_es_redirigido_al_registro(self):
        request = self._preparar_request(self.factory.get('/'), AnonymousUser())
        response = self.vista_de_prueba(request)
        self.assertEqual(response.status_code, 302)
        self.assertEqual(response.url, reverse('usuarios:registro'))

    def test_usuario_autenticado_sin_rol_admin_es_redirigido_a_tienda(self):
        cliente = crear_cliente()
        request = self._preparar_request(self.factory.get('/'), cliente)
        response = self.vista_de_prueba(request)
        self.assertEqual(response.status_code, 302)
        self.assertEqual(response.url, reverse('usuarios:tienda_home'))

    def test_usuario_admin_puede_pasar(self):
        admin = crear_admin()
        request = self._preparar_request(self.factory.get('/'), admin)
        response = self.vista_de_prueba(request)
